...
#define LH_DEFINE_OP0(effect, op, restype)                        \
  restype effect##_##op() {                                       \
    lh_value res = lh_yield(LH_OPTAG(effect, op), lh_value_null); \
    return lh_##restype##_value(res);                             \
  }

#define LH_DEFINE_OP1(effect, op, restype, argtype)                         \
  restype effect##_##op(argtype arg) {                                      \
    lh_value res = lh_yield(LH_OPTAG(effect, op), lh_value_##argtype(arg)); \
    return lh_##restype##_value(res);                                       \
  }
...
#define LH_WRAP_FUN0(fun, restype)    \
  lh_value wrap_##fun(lh_value arg) { \
    (void)(arg);                      \
    return lh_value_##restype(fun()); \
  }
...
#define implicit_declare(name)  \
  LH_DECLARE_EFFECT1(name, get) \
  LH_DECLARE_OP(name, get)
