...
export const taskQueueEvaluate = (
        ...
        switch (ast.value) {
          ...
          case "codeLabel": {
            ...
            const labelValue: TaskQueueFunctionValue = taskQueueFn(taskQueue, (value) => {
              taskQueue.send(context.continuation!, value);
              return null;
            });
            const scope = context.scope.add(label, { get: () => labelValue });

            return taskQueueEvaluate(taskQueue, expr, { ...context, scope });
          }
  ...
  if (context.continuation) taskQueue.pipe(res, context.continuation);
