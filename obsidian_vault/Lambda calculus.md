...
3. Применение или вызов функции, обозначаются двумя переменными подряд `x y`. Вызов функций лево-асоциативный, что значит `x y z = (x y) z` 
...
Например `fn x -> x` тогда можно записать например как `fn -> 0`, или `fn -> #0` что бы отличать эти индексы от обычных чисел
