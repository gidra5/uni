...
importantly if gives interpretation for expressions under patterns and patterns under expressions, which makes the whole system complete. Pattern values are naturally describing existentials over free variables. These also can be considered duals to functions - functions compute outputs from inputs, while patterns do the opposite and find inputs matching the output. The duality between two consumets of a single protocol looks awfully similar to this duality, and even visible in the way forall is introduced there, which is basically a polarity of information flow. But that holds only when functions are purely symbolic
